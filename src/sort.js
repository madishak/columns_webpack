        ...
        for (let i = 0; i < this.arrCopy.length-1; i++) {
            for (let j = this.currentIndex; j < this.arrCopy.length-i-1; j++) {
                if (this.arrCopy[j] > this.arrCopy[j + 1]) {
                    [this.arrCopy[j], this.arrCopy[j + 1]] = [this.arrCopy[j + 1], this.arrCopy[j]];
    ...
    decreaseSort() {
        for (let i = 0; i < this.arrCopy.length; i++) {
            if (this.listOfIndexes.length !== 0) {
                [this.arrCopy[this.listOfIndexes[this.listOfIndexes.length-1]], this.arrCopy[this.listOfIndexes[this.listOfIndexes.length-1] + 1]] =
                    [this.arrCopy[this.listOfIndexes[this.listOfIndexes.length-1] + 1], this.arrCopy[this.listOfIndexes[this.listOfIndexes.length-1]]];
